# swinir.py
# Simple Swin-style block implemented in PyTorch
# Window-based self-attention, shifted windows, LayerNorm, MLP, residuals
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        """
        x: (num_windows*B, N, dim) with N = window_size*window_size
        mask: None or (num_windows, N, N) with 0 and large negative entries
        returns: (num_windows*B, N, dim)
        """
        B_, N, C = x.shape
        qkv = self.qkv(x)
        ...
        if mask is not None:
            ...
            mask_expanded = mask.repeat(batch_repeat, 1, 1)  # (B_, N, N)
            attn = attn + mask_expanded.unsqueeze(1)  # broadcast to heads
...
class SwinIRBlock(nn.Module):
    def __init__(self,
        ...
        # small conv projection and reconstruction
    ...
    def forward(self, x):
        ...
        # embed
        x_emb = self.conv_embed(x)  # (B, embed_dim, H, W)
        x_perm = x_emb.permute(0, 2, 3, 1).contiguous()  # (B, H, W, C)
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size, self.embed_dim)  # (B_, N, C)

        # attention
        x_windows_norm = self.norm1(x_windows)
        attn_windows = self.attn(x_windows_norm, mask=self.attn_mask)  # (B_, N, C)
        ...
        # MLP branch
        x_mlp_prep = x_out.permute(0, 2, 3, 1).contiguous()  # (B, H, W, C)
        BHW = B * H * W
        x_mlp_flat = x_mlp_prep.view(BHW, self.embed_dim)
        x_mlp_norm = self.norm2(x_mlp_flat)
        x_mlp = self.mlp(x_mlp_norm)
        x_mlp = x_mlp.view(B, H, W, self.embed_dim).permute(0, 3, 1, 2).contiguous()  # (B, C, H, W)

        x_final = x_out + x_mlp

        # reconstruct and pixel residual
        x_recon = self.conv_unembed(x_final)  # (B, in_chans, H, W)
        out = x + x_recon
        return out
